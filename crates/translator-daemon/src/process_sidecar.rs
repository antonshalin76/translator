...
impl SidecarRuntime for ProcessSidecarRuntime {
    ...
    async fn shutdown_and_reap(&mut self) -> Result<ChildState, SupervisorError> {
        ...
        if process_group_exists(Some(pid))? {
            ...
            if !wait_for_group_exit(self.child.as_mut(), Some(pid), GRACEFUL_SHUTDOWN_TIMEOUT)
                .await?
            {
                return self.force_group_exit().await;
            }
        }
        if let Some(child) = self.child.as_mut() {
            child
    ...
    impl RetirementFixture {
        async fn ready() -> Self {
            ...
            let mut child = Command::new("/usr/bin/python3")
                ...
                .kill_on_drop(true)
                .spawn()
                .expect("spawn private model-free fixture");
            let pid = child.id().expect("fixture PID");
            let pidfd = pidfd_open(Pid::from_raw(pid as i32).unwrap(), PidfdFlags::empty())
                .expect("bind exact emergency cleanup identity before cancellation");
            let output = child.stdout.take().expect("fixture readiness pipe");
            let mut fixture = Self {
                runtime: ProcessSidecarRuntime {
                    python: PathBuf::from("/usr/bin/python3"),
                    sidecar_root: PathBuf::from("/tmp"),
                    socket_path: PathBuf::from("/tmp/d083-unused.sock"),
                    expected_uid: 0,
                    child: Some(child),
            ...
            if !matches!(ready, Ok(Ok(Some(ref line))) if line == "ready") {
                ...
                panic!("fixture signal policy was not installed");
            }
            fixture
        }

        fn snapshot(&self) -> (bool, Option<u32>, Option<u32>, Option<u32>, bool) {
            use rustix::fd::AsFd;
            ...
            (
                ...
                self.runtime.process_group_id,
                self.runtime.last_reaped_pid(),
                live,
            )
        }
        ...
        async fn cleanup(&mut self) {
            use rustix::fd::AsFd;
            use rustix::process::{WaitId, WaitIdOptions, pidfd_send_signal, waitid};

            // Repair only after saving the production oracles. Never signal a raw PID.
            self.runtime.process_group_id = None;
            let killed = pidfd_send_signal(&self.pidfd, Signal::KILL);
            let reaped = tokio::time::timeout(Duration::from_secs(2), async {
                ...
                loop {
                    match waitid(
                        ...
                        Ok(Some(_)) | Err(Errno::CHILD) => return true,
            ...
            .await;
            assert!(matches!(killed, Ok(()) | Err(Errno::SRCH)));
            assert_eq!(reaped, Ok(true), "exact fixture child was not reaped");
            self.runtime.child = None;
        }
    }

    #[cfg(target_os = "linux")]
    async fn cancelled_retirement_case(attempts: usize) {
        use std::future::{Future, poll_fn};
        use std::task::Poll;

        let mut fixture = RetirementFixture::ready().await;
        let mut observations = Vec::new();
        for _ in 0..attempts {
            let pending = {
                let shutdown = fixture.runtime.shutdown_and_reap();
                tokio::pin!(shutdown);
                poll_fn(|cx| Poll::Ready(shutdown.as_mut().poll(cx).is_pending())).await
                // The actual entered future is dropped here, without a reactor turn.
            };
            observations.push((pending, fixture.snapshot()));
        }
        let retry =
            tokio::time::timeout(Duration::from_secs(2), fixture.runtime.shutdown_and_reap()).await;
        let retired = fixture.snapshot();
        let group_absent = process_group_exists(Some(fixture.pid));
        let repeated =
            tokio::time::timeout(Duration::from_secs(2), fixture.runtime.shutdown_and_reap()).await;
        let after_repeated = fixture.snapshot();
        fixture.cleanup().await;
        ...
        assert_eq!(
            ...
            vec![
                (
                    true,
                    (true, Some(fixture.pid), Some(fixture.pid), None, true)
                );
                attempts
            ],
            "cancelled graceful retirement must retain the same live Child and PGID"
        );
        assert_eq!(retry, Ok(Ok(ChildState::Reaped)));
        assert_eq!(retired, (false, None, None, Some(fixture.pid), false));
        assert_eq!(group_absent, Ok(false));
        assert_eq!(repeated, Ok(Ok(ChildState::Reaped)));
        assert_eq!(after_repeated, retired);
    }

    #[cfg(target_os = "linux")]
    #[tokio::test]
    async fn d083_cancelled_graceful_retirement_retains_child() {
        cancelled_retirement_case(1).await;
    }

    #[cfg(target_os = "linux")]
    #[tokio::test]
    async fn d083_repeated_cancellation_preserves_exact_retry_owner() {
        cancelled_retirement_case(2).await;
    }

    #[cfg(target_os = "linux")]
    #[tokio::test]
    async fn d083_completed_forceful_retirement_is_idempotent() {
        let mut fixture = RetirementFixture::ready().await;
        let first =
            tokio::time::timeout(Duration::from_secs(2), fixture.runtime.kill_and_reap()).await;
        let retired = fixture.snapshot();
        let group_absent = process_group_exists(Some(fixture.pid));
        let second =
            tokio::time::timeout(Duration::from_secs(2), fixture.runtime.kill_and_reap()).await;
        ...
        assert_eq!(first, Ok(Ok(ChildState::Reaped)));
        assert_eq!(retired, (false, None, None, Some(fixture.pid), false));
        assert_eq!(group_absent, Ok(false));
        assert_eq!(second, Ok(Ok(ChildState::Reaped)));
        assert_eq!(after_second, retired);
    }
