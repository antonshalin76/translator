...
class Task9DesktopRunModeTests(unittest.TestCase):
    ...
    def test_start_accepts_safe_environment_without_disclosing_it(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            environment_file.write_text(f"TRANSLATOR_ASR_MODEL_ID={marker}\n")
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(
                environment_file.read_text(), f"TRANSLATOR_ASR_MODEL_ID={marker}\n"
            )
            observable_output = result.stdout + result.stderr + log_path.read_text()
            self.assertNotIn(marker, observable_output)

    def test_start_rejects_removed_headphone_override_before_systemd(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            env, environment_dir, environment_file, log_path = (
                self._desktop_security_test_environment(Path(tmp))
            )
            environment_dir.mkdir(mode=0o700)
            environment_file.write_text("TRANSLATOR_HEADPHONE_SINK=unverified-output\n")
            environment_file.chmod(0o600)
            ...
            self.assertNotIn("unverified-output", result.stdout + result.stderr)
    ...
    def test_daemon_wrapper_treats_command_syntax_as_literal_data(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            environment_file.write_text(f"TRANSLATOR_ASR_MODEL_ID=$(touch {marker})\n")
            environment_file.chmod(0o600)
            ...
            self.assertIn(f"TRANSLATOR_ASR_MODEL_ID=$(touch {marker})", result.stdout)
    ...
    def test_daemon_wrapper_rejects_ambiguous_environment_syntax(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            daemon = self._install_test_daemon(tmp_path)
            invalid_documents = (
                b"TRANSLATOR_ASR_MODEL_ID=one\nTRANSLATOR_ASR_MODEL_ID=two\n",
                b"export TRANSLATOR_ASR_MODEL_ID=value\n",
                b"TRANSLATOR_ASR_MODEL_ID=\xff\n",
                b"#\n" * 256,
            )
