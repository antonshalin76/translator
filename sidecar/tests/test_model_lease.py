...
from translator_sidecar.local import model_lease
...
def test_sealed_directory_ignores_cache_replacement_and_closes_with_lease(
    ...
    source = model_source(
        tmp_path,
        {"model.bin": b"approved", "config.json": b"{}"},
    )
    ...
    try:
        ...
        replacement = tmp_path / "replacement"
        replacement.write_bytes(b"attacker")
        replacement.replace(tmp_path / "model.bin")
        assert (directory / "model.bin").read_bytes() == b"approved"
        assert (directory / "config.json").read_bytes() == b"{}"
...
def test_whisper_directory_mutation_and_ambient_config_cannot_reopen_cache(tmp_path):
    ...
    def factory(identifier, **kwargs):
        ...
        assert (directory / "tokenizer.json").read_bytes() == b"{}"
        assert not (directory / "preprocessor_config.json").exists()
        assert (directory / "model.bin").read_bytes() == b"approved"
    ...
    asr.prepare()
    assert (Path(observed[0]) / "model.bin").read_bytes() == b"approved"
    asr.close()
    assert not Path(observed[0]).exists()


def test_asr_directory_link_failure_closes_acquired_lease(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = model_source(tmp_path, {"model.bin": b"approved", "tokenizer.json": b"{}"})
    original_symlink = os.symlink

    def fail_tokenizer(target: str, link_name: str | Path) -> None:
        if Path(link_name).name == "tokenizer.json":
            raise OSError("link failed")
        original_symlink(target, link_name)

    monkeypatch.setattr(os, "symlink", fail_tokenizer)
    monkeypatch.setattr(
        model_lease,
        "TemporaryDirectory",
    ...
    gc.collect()
    assert len(os.listdir("/proc/self/fd")) == before
    assert not list(tmp_path.glob("translator-model-*"))


def test_asr_loader_uses_sealed_directory_without_python_model_copy(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = model_source(
        tmp_path,
    ...
    def record_read(self: VerifiedModelLease, name: str) -> bytes:
        reads.append(name)
        raise AssertionError("ASR must not copy verified model bytes")

    def factory(identifier: str, **kwargs: object):
        assert kwargs.get("files") is None
        directory = Path(identifier)
        assert directory.is_dir()
        assert (directory / "model.bin").read_bytes() == b"approved"
        ...
        ).read_bytes() == b'{"feature_size":80}'
        return NativeModel()

    monkeypatch.setattr(VerifiedModelLease, "read_bytes", record_read)
    manager = AsrModelManager(
        selected_id="small",
        model_paths={"small": source},
        device="cpu",
...
def test_asr_retained_inner_native_keeps_snapshot_and_blocks_release(tmp_path):
    ...
    assert (Path(paths[0]) / "model.bin").read_bytes() == b"model"
...
def test_installed_whisper_consumes_verified_directory_with_library_defaults(
    tmp_path, monkeypatch
):
    import ctranslate2
    import tokenizers
    ...
    class NativeWhisper(NativeModel):
        def __init__(self, identifier, *, files, **kwargs):
            ...
            ).read_bytes() == tokenizer.to_str().encode()
            assert not (directory / "preprocessor_config.json").exists()
            observed.append(identifier)
