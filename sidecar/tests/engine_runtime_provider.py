...
class EngineRuntimeProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self._now_ns = now_ns
        self._publishers: dict[UUID, PublishEvents] = {}
    ...
    async def open_session(
        ...
        opened = self._call(self.engine.open_session, request)
        self._publishers[request.session_id] = publish
        try:
            health = self._call(
                self.engine.health,
                request.session_id,
            ...
            self._publishers.pop(request.session_id, None)
            self._call(self.engine.release_session, request.session_id)
            raise
    ...
    async def close_session(self, request: CloseProviderSession) -> None:
        try:
            event = self._call(self.engine.close_session, request)
            await self._publish(request.session_id, (event,))
        finally:
            self._publishers.pop(request.session_id, None)
            self._call(self.engine.release_session, request.session_id)
    ...
    async def shutdown(self) -> None:
        for session_id in tuple(self._publishers):
    ...
    async def _publish(
        ...
        publish = self._publishers.get(session_id)
        if publish is None:
            raise LocalProviderProtocolError("session is unavailable")
        await publish(events, lambda: None)
