...
class SessionDrainReceipt:
    session_id: UUID
    delivery_error: SafeErrorCode | None = None


class RuntimeSessionReservation(Protocol):
...
class ProviderLease:
    ...
    async def release(self) -> None:
        ...
        self._registry._admit_retirement_retry(self._entry)
...
class ProviderRegistry:
    ...
    def _admit_retirement_retry(self, entry: _Entry) -> None:
        if (
            entry.disposal is not None
            and entry.disposal.done()
            and (entry.disposal.cancelled() or entry.disposal.exception() is not None)
        ):
            entry.disposal = None
    ...
    async def _retire(self, entry: _Entry) -> None:
        ...
        attempt = entry.disposal
        try:
            await finish_cleanup(attempt)
        finally:
            if (
                entry.disposal is attempt
                and attempt.done()
                and not attempt.cancelled()
                and attempt.exception() is None
            ):
    ...
    async def shutdown(self) -> None:
        ...
        for entry in self._entries.values():
            self._admit_retirement_retry(entry)
        await self.collect()
